...
class balance(minqlx.Plugin):
    ...
    def __init__(self):
        ...
        # Keys: steam_id - Items: true/false
        self.majority_voters = {}
        ...
        self.set_cvar_once("qlx_majority_vote_mode", "0")
		self.set_cvar_once("qlx_majority_vote_threshold", "0.5")
	...
	def handle_team_switch(self, player, old, new):
		if old in ["red", "blue"] and new == "spectator":
			self.handle_player_disconnect(player, None)
	
    def handle_player_disconnect(self, player, reason):
		# If a player disconnects and was in the majority_voters var, then a suggestion should be active
		#  Thus we should check if we have the majority now
        if player.steam_id in self.majority_voters:
            del self.majority_voters[player.steam_id]
			needed = self.calculate_majority_needed()
			if needed <= 0:
				self.msg("^7We've just passed the majority threshold. Initiating switch...")
    ...
    def callback_teams(self, players, channel):
        ...
        if switch and switch[1] >= minimum_suggestion_diff:
            option_message = "Mentioned players can type !a to agree"
            if self.get_cvar("qlx_majority_vote_mode", int) == 1:
				option_message += ", others can type !a to vote."
            elif self.get_cvar("qlx_majority_vote_mode", int) == 2:
                option_message = "All players can type !a to vote."
            channel.reply("SUGGESTION: switch ^6{}^7 with ^6{}^7. " + option_message
            ...
            if not self.suggested_pair or self.suggested_pair[0] != switch[0][0] or self.suggested_pair[1] != switch[0][1]:
                ...
                self.suggested_agree = [False, False]
				# give every player an option to vote
				self.majority_voters = dict([(p.steam_id, 0) for p in teams["red"] + teams["blue"])
        else:
    ...
    def cmd_agree(self, player, msg, channel):
        ...
        if self.suggested_pair and not all(self.suggested_agree):
			...
			# If we are not in strict mode (mode 2 == only upon majority), check if the players agreed themselves
			if self.get_cvar("qlx_majority_vote_mode", int) != 2:
				if all(self.suggested_agree):
					# IF they both agreed, do the switch and we're done
					do_the_switch()
					return 				
			
			# If the players themselves did not agree yet, check for majority voting	
			if self.get_cvar("qlx_majority_vote_mode", int) > 0:
							
				# Spectators may not vote !a
				if player.team == "spectator":
					channel.reply("^7Spectators may not participate in the majority vote, {}".format(player.name))
					return
				
				# Only people in the list are allowed to vote
				if player.steam_id not in self.majority_voters:
					channel.reply("^7You may not participate in the majority vote, {}".format(player.name))
					return
					
				# Players may only vote once
				if self.majority_voters[player.steam_id]:
					channel.reply("^7You already voted for this switch, {}".format(player.name))
					return
				
				# Register the vote
				self.majority_voters[player.steam_id] += 1
				
				# Check if threshold is reached
				votes_needed = self.calculate_majority_needed()
				
				# if more votes are needed, display how many 
				if votes_needed > 0:
					self.msg("^7{}^7 agreed, but I need ^6{}^7 more to reach the {} percent threshold.".format(player.name, votes_needed, int(threshold*100))
					return
					
				# Otherwise, apply the switch
				do_the_switch()
				
				
					
				
	def calculate_majority_needed(self):	
		threshold = self.get_cvar("qlx_majority_vote_threshold", float)
		votes = list(self.majority_voters.values())
		return int(votes.count(0) - len(votes) * (1 - threshold) + 1)
